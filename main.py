...
class BuildingInfoFloorStore(BaseModel):
    name: str = ""
    category: str = ""


class BuildingInfoFloorItem(BaseModel):
    floor: str = ""
    stores: list[BuildingInfoFloorStore] = []
...
class BuildingInfoResponse(BaseModel):
    ...
    address: str = ""
    phone: str = ""
    floor_info: list[BuildingInfoFloorItem] = []


@app.get("/place/building/{ufid}", response_model=BuildingInfoResponse)
async def place_building_info(ufid: str):
    """건물 핀 탭 시 Supabase place_info 기반 건물 정보 + 층별 매장 정보 반환 (VWorld ufid 직접 조회)."""
    from agents.place_insight_agent import _fetch_place_info
    place_data = await _fetch_place_info(ufid)
    if not place_data:
        return BuildingInfoResponse(ufid=ufid, found=False)
    return _build_building_response(ufid, place_data)


@app.get("/place/building/by-bd/{bd_mgt_sn}", response_model=BuildingInfoResponse)
async def place_building_info_by_bd(bd_mgt_sn: str):
    """bd_mgt_sn으로 VWorld ufid를 찾아 place_info 조회. 건물 핀 클릭 시 프론트엔드 메인 경로."""
    from agents.place_insight_agent import _fetch_place_info, _fetch_building_meta_by_bd_mgt_sn, _trigger_background_pipeline
    from tools.building_raycast import fetch_building_by_bd_mgt_sn

    # 1) VWorld JSON 직접 조회 (가장 빠름)
    vworld_meta = fetch_building_by_bd_mgt_sn(bd_mgt_sn)
    bld_nm_fallback = ""
    if vworld_meta:
        ufid = vworld_meta.get("ufid") or ""
        bld_nm_fallback = vworld_meta.get("bld_nm") or ""
    else:
        # 2) buildings DB 좌표 근접 조회 → VWorld ufid
        ufid, bld_nm_fallback = await _fetch_building_meta_by_bd_mgt_sn(bd_mgt_sn)

    if not ufid:
        return BuildingInfoResponse(ufid="", found=False, name=bld_nm_fallback)

    place_data = await _fetch_place_info(ufid)
    if not place_data:
        _trigger_background_pipeline(ufid)
        return BuildingInfoResponse(ufid=ufid, found=False, name=bld_nm_fallback)

    return _build_building_response(ufid, place_data, name_fallback=bld_nm_fallback)


def _build_building_response(ufid: str, place_data: dict, name_fallback: str = "") -> BuildingInfoResponse:
    floor_raw = place_data.get("floor_info") or []
    floor_info = [
        BuildingInfoFloorItem(
            floor=str(f.get("floor", "")) if isinstance(f, dict) else "",
            stores=[
                BuildingInfoFloorStore(
                    name=str(s.get("name", "")),
                    category=str(s.get("category", "")),
                )
                for s in (f.get("stores", []) if isinstance(f, dict) else [])
    ...
    return BuildingInfoResponse(
        ufid=ufid,
        found=True,
        name=place_data.get("name_ko") or name_fallback or "",
        category=place_data.get("category") or "",
        description_ko=place_data.get("description_ko") or place_data.get("description_en") or "",
        open_hours=place_data.get("open_hours") or "",
        closed_days=place_data.get("closed_days") or "",
        parking_info=place_data.get("parking_info") or "",
        admission_fee=place_data.get("admission_fee") or "",
        homepage=place_data.get("homepage") or "",
        image_url=place_data.get("image_url") or "",
        address=place_data.get("address") or place_data.get("addr") or "",
        phone=place_data.get("phone") or "",
