...
buildings DB 커버리지 기반으로 관광지·문화시설을 수집해 tourist_places 테이블에 적재.

seed_from_buildings.py 와 동일한 H3 클러스터 방식으로 Kakao AT4(관광명소) + CT1(문화시설)을
검색하고, 각 장소에 대해 Tour API로 개요·입장료·운영시간·주차 정보를 보강한다.

사용:
    python scripts/seed_tourist_places.py
    python scripts/seed_tourist_places.py --per-category 45 --radius 2000
        ...
        for c_idx, (cell, lat, lng, bld_cnt) in enumerate(clusters, 1):
            ...
            for cat, code in CATEGORY_CODES.items():
                ...
                for i, d in enumerate(docs, 1):
                    ...
                    place_id = f"tourist__{kakao_id}" if kakao_id else f"tourist__{name}"
                    ...
                    cat_key = "tourist" if cat == "tourist" else "cultural"
                    ...
                    total.append({"name": name, "cat": cat, "enriched": bool(enriched)})
                    print(f"    [{i:2d}] {name}  source={enriched.get('source', '-')}  입장료={details.get('admission_fee', '')!r}")
